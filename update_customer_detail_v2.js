const fs = require('fs');
const path = 'src/screens/manager/CustomerDetailScreen.js';
...
// 3. Functions
const funcStart = content.indexOf('const generateBatchHTMLString =');
const funcEnd = content.indexOf('const renderDateSelector =');

if (funcStart !== -1 && funcEnd !== -1) {
    const newFuncs = `    const handleExportImage = async () => {
...
// 4. Update the export buttons
const p1 = content.split('<View style={styles.exportButtonsRow}>');
if (p1.length > 1) {
    const p1_before = p1[0];
    const p1_after = p1[1];
    const endOfRow = p1_after.indexOf('</View>') + 7;

    const newButtons = `<View style={styles.exportButtonsRow}>
                                            <TouchableOpacity
    ...
    content = p1_before + newButtons + p1_after.substring(endOfRow);
}

// 5. Add Hidden Receipt
